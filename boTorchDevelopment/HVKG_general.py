...
class HVKG:
    def __init__(self, function, n_var, n_obj, refVectorValue):
        ...
        self.refVector = np.full((self.n_obj,), refVectorValue)
        self.BATCH_SIZE = 1
        ...
        self.COST_BUDGET = 110 if not os.environ.get("SMOKE_TEST") else 54
    ...
    def optimize_HVKG_and_get_obs_decoupled(self, model, cost_model, standard_bounds, objective_indices):
        ...
        # observe new values
        new_x = unnormalize(candidates.detach(), bounds=self.bounds_reversed)
        new_obj = self.evalPyMooProblem(new_x.cpu().numpy())
    ...
    def get_model_identified_hv_maximizing_set(
        ...
        X = unnormalize(X, self.bounds_reversed)
        # print(X, X.shape)
        # Y = problem(X)
    ...
    def optimise(self):
        ...
        self.problem, self.bounds = self.getPyMooProblem()
        self.bounds = torch.from_numpy(self.bounds)
